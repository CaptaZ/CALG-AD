...
class ALGA_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x):
        ...
        kv = torch.einsum('...sd,...se->...de', k, v)
        z = 1.0 / torch.einsum('...sd,...d->...s', q, k.sum(dim=-2))
        x_global = torch.einsum('...de,...sd,...s->...se', kv, q, z)
        x_global = x_global.transpose(1, 2).reshape(B, N, C)

        H = int(math.sqrt(N))
        num_spatial = H * H
        num_extra = N - num_spatial

        v_reshaped = v.transpose(1, 2).reshape(B, N, C)
        v_extra = v_reshaped[:, :num_extra, :]
        v_spatial = v_reshaped[:, num_extra:, :]

        v_spatial_2d = v_spatial.transpose(1, 2).reshape(B, C, H, H)
        v_spatial_conv_2d = self.dwconv(v_spatial_2d)

        v_spatial_conv = v_spatial_conv_2d.flatten(2).transpose(1, 2)
        v_local = torch.cat([v_extra, v_spatial_conv], dim=1)

        local_gate = self.dynamic_router(x_global)

        x_fused = x_global + local_gate * v_local
