...
def main(item_list, args):
    ...
    bottleneck.append(bMlp(embed_dim, 512, embed_dim, drop=0.2))
    ...
    for i in range(8):
        blk = VitBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=4.,
                       ...
                       attn=ALGA_Attention if args.alga else LinearAttention2)
    ...
    model = ViTill(encoder=encoder, bottleneck=bottleneck, decoder=decoder, target_layers=target_layers,
                   ...
                   adaptive_context_decouple=args.acd)
    model = model.to(device)

    flops, macs, params = get_model_profile(model=model, input_shape=(1, 3, args.crop_size, args.crop_size),
                                            detailed=False, as_string=True)
    print_fn(f"Model: FLOPs={flops}, MACs={macs}, Params={params}")

    if args.phase == 'train':
        trainable = nn.ModuleList([bottleneck, decoder])
        if args.acd:
            trainable.append(model.context_alpha)
        ...
        model_path = os.path.join(os.path.join(args.save_dir, args.save_name), 'model.pth')
        print_fn(f"Loading weights from: {model_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='../../data/mvtec_anomaly_detection')  # organized_ZJU-Leaper2
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='MVTec_textile')
    parser.add_argument('--total_iters', type=int, default=2000)
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--image_size', type=int, default=448)
    parser.add_argument('--crop_size', type=int, default=392)
    parser.add_argument('--acd', type=int, default=1)
    parser.add_argument('--alga', type=int, default=1)
    ...
    elif 'wfdd' in args.data_path.lower():
        item_list = ['grey_cloth', 'grid_cloth', 'pink_flower', 'yellow_cloth']
    elif 'zju' in args.data_path.lower():
        item_list = ['white_plain', 'brown_plaid', 'gray_plaid', 'red_plaid', 'floral_print1',
                     'floral_print2', 'floral_print3', 'thick_stripe', 'thin_stripe', 'dot_pattern',
                     'houndstooth', 'gingham', 'knot_pattern', 'twill_plaid', 'blue_plaid']
    else:
